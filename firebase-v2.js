(() => {
  ...
  function friendlyAuthError(err){
    const code=err?.code||'ukjent-feil';
    const map={
      'auth/invalid-credential':'Firebase avviser e-post eller passord.',
      'auth/invalid-login-credentials':'Firebase avviser e-post eller passord.',
      'auth/user-not-found':'Firebase finner ikke denne brukeren.',
      'auth/wrong-password':'Passordet stemmer ikke.',
      'auth/unauthorized-domain':'Domenet er ikke godkjent i Firebase.',
      'auth/operation-not-allowed':'E-post/passord er ikke aktivert som innloggingsmetode.',
      'auth/network-request-failed':'Nettleseren fikk ikke kontakt med Firebase.',
      'auth/invalid-api-key':'Firebase API-nøkkelen i appen er ugyldig.'
    };
    return (map[code]||err?.message||'Kunne ikke logge inn.')+' Feilkode: '+code;
  }
